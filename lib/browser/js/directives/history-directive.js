app.directive('history', function (HistoryFactory) {
    ...
    return {
        ...
        link: function (scope) {
            ...
            scope.theFile;
            console.log("file on the scope", scope.theFile);
            scope.$watch('theFile', function(newVal, oldVal) {
                console.log("newVal", newVal);
            })

            // scope.uploadFile = function(files) {
            //     HistoryFactory.uploadFile(files).then(function(history))
            // }


            uploadFile = function(files) {
                //console.log("history", history);
                console.log("files - theFile ", files);
                var file = files[0];
                ...
                fr.onload = function() {
                   ...
                   console.log("laoded", loadedHistory);
                   console.log("typeof loaded Hist", loadedHistory instanceof Array);
                   // var parsedHist = JSON.parse(loadedHistory);
                   // console.log("typeof", typeof parsedHist);
                   HistoryFactory.concatHistory(loadedHistory);
                   console.log("history", HistoryFactory.getHistory())
